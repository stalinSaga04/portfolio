            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {skillCategories.map((category) => (
                        <div
                            key={category.title}
                            className="bg-white p-8 rounded-[2.5rem] border border-slate-200 hover:border-indigo-300 transition-all duration-500 hover:scale-[1.03] group shadow-sm hover:shadow-2xl hover:shadow-indigo-100/50"
                        >
                            <div className="w-16 h-16 bg-indigo-600/10 text-indigo-600 rounded-2xl flex items-center justify-center mb-8 border border-indigo-200/50 group-hover:rotate-12 transition-all duration-500 shadow-inner">
                                {category.icon}
                            </div>
                            <h3 className="text-2xl font-black text-slate-900 mb-6 tracking-tight">{category.title}</h3>
                            <div className="flex flex-wrap gap-2.5">
                                {category.skills.map((skill) => (
                                    <span
                                        key={skill}
                                        className="px-4 py-2 rounded-xl bg-slate-50 text-slate-600 text-[11px] md:text-xs font-black border border-slate-100 group-hover:border-indigo-200 group-hover:bg-indigo-50 transition-all duration-300"
                                    >
                                        {skill}
                                    </span>
                                ))}
